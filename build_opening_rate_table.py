def build_opening_rate_table(act, grouped_parameter, is_percent=True):
    import pandas as pd
    import numpy as np
    import string
    ...
    for opt in all_rows_options:
        df_openers = act[(act.action == 'view') 
                        & (act.page_type == 'product')
                        & (act[grouped_parameter] == opt)].groupby('experiment_name').uid.nunique()
        dfs_list.append(pd.Series(df_openers, name = opt))
    #grouped df
