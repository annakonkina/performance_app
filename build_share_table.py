def build_share_table(act, cart, metric, grouped_parameter, is_percent=True):
    import pandas as pd
    import numpy as np
    import string
    ...
    for opt in all_rows_options:
        df_buyers = cart[(cart.uid.isin(all_buyers)) & (cart[grouped_parameter] == opt)].groupby('experiment_name')[metric].sum()
    ...
    for opt in grouped_df.index.values:
        ...
        for exp in grouped_df.columns:
            if not is_percent:
                ...
                nb = int(grouped_df.loc[opt, exp]*base_row.loc[metric, exp]/100)
                
            stat_test_dict[opt][exp] = {
                'number':nb,
                'base':base_row.loc[metric, exp],
                'stat':[],
            }
    ...
    base_row.loc[metric, :] = base_row.loc[metric, :].apply(lambda x: np.round(x, 2))
    
    #concatenation
    grouped_df = pd.concat([base_row, grouped_df])
    grouped_df = grouped_df.drop(index = ['experiment_name']).rename(index = {metric:'Base'})
